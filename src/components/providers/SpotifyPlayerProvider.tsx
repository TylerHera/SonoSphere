...
import { updateNowPlaying as lastfmUpdateNowPlaying, scrobbleTrack as lastfmScrobbleTrack } from '@/lib/api/lastfm';
...
export const SpotifyPlayerProvider: React.FC<SpotifyPlayerProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    window.onSpotifyWebPlaybackSDKReady = () => {
      ...
      newPlayer.addListener('not_ready', ({ device_id }) => {
        ...
        setPlayerState(prev => ({ ...prev, deviceId: device_id, isReady: false, isActive: false }));
      });

      newPlayer.addListener('player_state_changed', (state: Spotify.PlaybackState | null) => {
        if (!state) {
          setPlayerState(prev => ({ ...prev, isActive: false, isPlaying: false, currentTrack: null }));
          return;
        }
        const currentTrackChanged = playerState.currentTrack?.uri !== state.track_window.current_track.uri;
        setPlayerState(prev => ({
          ...prev,
          isActive: true,
        ...
        if (currentTrackChanged || (!state.paused && playerState.isPlaying !== !state.paused)) {
            handleScrobbling(state.track_window.current_track, !state.paused, state.position, state.duration);
        }
      });
  ...
  const handleScrobbling = useCallback((track: Spotify.Track, isPlaying: boolean, position: number, duration: number) => {
    ...
    lastfmUpdateNowPlaying(
      ...
      if ('error' in response) toast.error(`Last.fm Now Playing Error: ${response.message}`);
    });

    // Set timeout for scrobbling
    // Don't scrobble if already scrobbled or track is too short
    if (lastTrackScrobbledRef.current === track.uri || duration < 30000) { // Less than 30s
        return;
    }

    const timeToScrobble = Math.min(duration * SCROBBLE_PERCENTAGE, SCROBBLE_MAX_TIME) - position;
    if (timeToScrobble > 0) {
      scrobbleTimeoutRef.current = setTimeout(() => {
        const currentTimestamp = Math.floor(Date.now() / 1000) - Math.floor(duration / 1000); // Timestamp when track started
        lastfmScrobbleTrack(
          track.artists[0].name,
          track.name,
          currentTimestamp,
  ...
  useEffect(() => {
    if (playerState.currentTrack?.uri !== lastTrackScrobbledRef.current && playerState.currentTrack) {

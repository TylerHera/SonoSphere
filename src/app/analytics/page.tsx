...
export default function AnalyticsPage() {
  ...
  const [topArtists, setTopArtists] = useState<
    AnalyticsData<LastFmArtistStatsFE>
  >({ items: [], total: 0 });
  const [topTracks, setTopTracks] = useState<AnalyticsData<LastFmTrackStatsFE>>(
    { items: [], total: 0 },
  );
  const [topAlbums, setTopAlbums] = useState<AnalyticsData<LastFmAlbumStatsFE>>(
    { items: [], total: 0 },
  );
  ...
  const [weeklyDataTimeframe, setWeeklyDataTimeframe] = useState<{
    from: string;
    to: string;
  } | null>(null);
    ...
    async (period: Period) => {
      ...
      try {
        ...
        const [artistsRes, tracksRes, albumsRes] = await Promise.all([
          fetch(
            `${apiBaseUrl}/analytics/top-artists?period=${period}&limit=${limit}`,
            { headers },
          ),
          fetch(
            `${apiBaseUrl}/analytics/top-tracks?period=${period}&limit=${limit}`,
            { headers },
          ),
          fetch(
            `${apiBaseUrl}/analytics/top-albums?period=${period}&limit=${limit}`,
            { headers },
          ),
        ]);
        ...
        if (!artistsRes.ok)
          throw new Error(
            `Failed to fetch top artists: ${artistsRes.statusText} (status: ${artistsRes.status})`,
          );
        if (!tracksRes.ok)
          throw new Error(
            `Failed to fetch top tracks: ${tracksRes.statusText} (status: ${tracksRes.status})`,
          );
        if (!albumsRes.ok)
          throw new Error(
            `Failed to fetch top albums: ${albumsRes.statusText} (status: ${albumsRes.status})`,
          );
        ...
        if (artistsData.error || tracksData.error || albumsData.error) {
          ...
          const errorMessage =
            ...
            tracksData.message ||
            albumsData.message ||
            'An error occurred in the backend or Last.fm API.';
        ...
        const [weeklyTracksRes, weeklyArtistsRes, weeklyAlbumsRes] =
          await Promise.all([
            fetch(
              `${apiBaseUrl}/analytics/weekly-tracks?limit=${weeklyLimit}`,
              { headers },
            ), // limit might not be supported by Last.fm API this way, but we take top N from response
            fetch(
              `${apiBaseUrl}/analytics/weekly-artists?limit=${weeklyLimit}`,
              { headers },
            ),
            fetch(
              `${apiBaseUrl}/analytics/weekly-albums?limit=${weeklyLimit}`,
              { headers },
            ),
          ]);

        if (!weeklyTracksRes.ok)
          throw new Error(
            `Failed to fetch weekly tracks: ${weeklyTracksRes.statusText} (status: ${weeklyTracksRes.status})`,
          );
        if (!weeklyArtistsRes.ok)
          throw new Error(
            `Failed to fetch weekly artists: ${weeklyArtistsRes.statusText} (status: ${weeklyArtistsRes.status})`,
          );
        if (!weeklyAlbumsRes.ok)
          throw new Error(
            `Failed to fetch weekly albums: ${weeklyAlbumsRes.statusText} (status: ${weeklyAlbumsRes.status})`,
          );
        ...
        if (
          weeklyTracksData.error ||
          weeklyArtistsData.error ||
          weeklyAlbumsData.error
        ) {
          const weeklyError =
            weeklyTracksData.message ||
            weeklyArtistsData.message ||
            weeklyAlbumsData.message ||
        ...
        setWeeklyArtists(
          (weeklyArtistsData.artists || []).slice(0, weeklyLimit),
        );
        ...
        if (weeklyTracksData.from && weeklyTracksData.to) {
          setWeeklyDataTimeframe({
            from: weeklyTracksData.from,
            to: weeklyTracksData.to,
          });
        } else if (weeklyArtistsData.from && weeklyArtistsData.to) {
          setWeeklyDataTimeframe({
            from: weeklyArtistsData.from,
            to: weeklyArtistsData.to,
          });
        } else if (weeklyAlbumsData.from && weeklyAlbumsData.to) {
          setWeeklyDataTimeframe({
            from: weeklyAlbumsData.from,
            to: weeklyAlbumsData.to,
          });
        }
      } catch (e: any) {
  ...
  useEffect(() => {
    if (user && session) {
      // Ensure user and session are available
    ...
    return (
      <div className="p-4 md:p-6 text-center text-destructive">
        ...
        {error.includes('Last.fm username not found') && (
          <Button asChild className="mt-4">
            <Link href="/settings/connections">Connect Last.fm</Link>
          </Button>
        )}
      </div>
    );
    ...
    <div className="space-y-8 p-4 md:p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            {isLoading && topArtists.items.length === 0 && (
              <p className="text-muted-foreground">Loading artists...</p>
            )}
            {!isLoading && topArtists.items.length === 0 && !error && (
        ...
        <Card>
          ...
          <CardContent>
            {isLoading && topTracks.items.length === 0 && (
              <p className="text-muted-foreground">Loading tracks...</p>
            )}
            {!isLoading && topTracks.items.length === 0 && !error && (
              <p className="text-muted-foreground">
              ...
              {topTracks.items.map((track, index) => (
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      by {track.artist['#text']} - {track.playcount} plays
                    </p>
                  </div>
                </li>
              ))}
        ...
        <Card>
          ...
          <CardContent>
            {isLoading && topAlbums.items.length === 0 && (
              <p className="text-muted-foreground">Loading albums...</p>
            )}
            {!isLoading && topAlbums.items.length === 0 && !error && (
              <p className="text-muted-foreground">
                No album data available for this period.
              </p>
            )}
            <ul className="space-y-3">
              {topAlbums.items.map((album, index) => (
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      by {album.artist['#text']} - {album.playcount} plays
                    </p>
                  </div>
                </li>
              ))}
      ...
      {weeklyDataTimeframe && (
        <div className="mt-8">
          <h2 className="text-2xl font-semibold mb-4">
            ...
            {new Date(
              parseInt(weeklyDataTimeframe.from) * 1000,
            ).toLocaleDateString()}{' '}
            -{' '}
            {new Date(
              parseInt(weeklyDataTimeframe.to) * 1000,
            ).toLocaleDateString()}
            )
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <Card>
              ...
              <CardContent>
                {isLoading && weeklyArtists.length === 0 && (
                  <p className="text-muted-foreground">Loading...</p>
                )}
                {!isLoading && weeklyArtists.length === 0 && !error && (
            ...
            <Card>
              ...
              <CardContent>
                {isLoading && weeklyTracks.length === 0 && (
                  <p className="text-muted-foreground">Loading...</p>
                )}
                {!isLoading && weeklyTracks.length === 0 && !error && (
                  <p className="text-muted-foreground">No data.</p>
                )}
                <ul className="space-y-3">
            ...
            <Card>
              ...
              <CardContent>
                {isLoading && weeklyAlbums.length === 0 && (
                  <p className="text-muted-foreground">Loading...</p>
                )}
                {!isLoading && weeklyAlbums.length === 0 && !error && (
                  <p className="text-muted-foreground">No data.</p>
                )}
                <ul className="space-y-3">

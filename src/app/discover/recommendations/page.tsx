...
export default function RecommendationsPage() {
  ...
  const [currentDiscoveryQueue, setCurrentDiscoveryQueue] =
    useState<DiscoveryQueue | null>(null);
  ...
  const [targetDanceability, setTargetDanceability] = useState<
    number | undefined
  >(undefined);
  const [targetEnergy, setTargetEnergy] = useState<number | undefined>(
    undefined,
  );
  const [targetValence, setTargetValence] = useState<number | undefined>(
    undefined,
  );
  ...
  const totalSeeds =
    ...
    seedGenresInput.split(',').filter((g) => g.trim() !== '').length;
  ...
  const debouncedArtistSearch = useCallback(
    debounce(async (query: string, token: string) => {
      ...
      try {
        const results: SpotifySearchResults = await searchSpotify(
          token,
          query,
          ['artist'],
          5,
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (totalSeeds > MAX_SEEDS) {
      toast.error(
        `Too many seeds. Maximum of ${MAX_SEEDS} allowed, found ${totalSeeds}. Please remove some.`,
      );
      return;
    }
    ...
    try {
      const params: SpotifyRecommendationsParams = {
        limit: limit,
        seed_artists:
          selectedArtistSeeds.map((s) => s.id).join(',') || undefined,
      ...
      if (targetDanceability !== undefined)
        params.target_danceability = targetDanceability;
      ...
      const response = await getSpotifyRecommendations(
        ...
        sourceSeeds: {
          artists:
            selectedArtistSeeds.length > 0
              ...
              : undefined,
          tracks:
            selectedTrackSeeds.length > 0 ? [...selectedTrackSeeds] : undefined,
          genres:
            seedGenresInput.split(',').filter((g) => g.trim() !== '').length > 0
              ? seedGenresInput.split(',').filter((g) => g.trim() !== '')
              : undefined,
        },
      };
    ...
    <div className="container mx-auto py-8">
      ...
      <Card className="mb-8">
        <CardHeader>
          ...
          <CardDescription>
            Add up to {MAX_SEEDS} seeds (artists, tracks, or genres) and
            optionally tune parameters to get recommendations.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              <Label htmlFor="artistSearch">
                Seed Artists ({selectedArtistSeeds.length}/
                {MAX_SEEDS -
                  (selectedTrackSeeds.length +
                    (seedGenresInput.split(',').filter((g) => g.trim() !== '')
                      .length || 0))}{' '}
                ...
                <PopoverTrigger asChild>
                  <div className="relative">
                    <Input
                      ...
                      disabled={
                        totalSeeds >= MAX_SEEDS &&
                        !selectedArtistSeeds.find((a) =>
                          a.name
                            .toLowerCase()
                            .includes(artistSearchQuery.toLowerCase()),
                        )
                      }
                    />
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {selectedArtistSeeds.map((artist) => (
                  <Badge key={artist.id} variant={'secondary' as any}>
                    {artist.name}
                    <Button
                      variant={'ghost' as any}
                      size="icon"
                      className="ml-2 h-4 w-4"
                      onClick={() => removeSeed(artist.id, 'artist')}
                    >
            ...
            <div className="space-y-2">
              <Label htmlFor="trackSearch">
                Seed Tracks ({selectedTrackSeeds.length}/
                {MAX_SEEDS -
                  (selectedArtistSeeds.length +
                    (seedGenresInput.split(',').filter((g) => g.trim() !== '')
                      .length || 0))}{' '}
                    ...
                    <Command>
                      <CommandList>
                        ...
                        {!isSearchingTracks &&
                          ...
                          trackSearchQuery && (
                            <CommandItem disabled>No tracks found.</CommandItem>
                          )}
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {selectedTrackSeeds.map((track) => (
                  <Badge key={track.id} variant={'secondary' as any}>
                    {track.name}
                    <Button
                      variant={'ghost' as any}
                      size="icon"
                      className="ml-2 h-4 w-4"
                      onClick={() => removeSeed(track.id, 'track')}
                    >
                      <XIcon className="h-3 w-3" />
                    </Button>
                  </Badge>
            ...
            <div>
              <Label htmlFor="seed_genres_input">
                Seed Genres (comma-separated)
              </Label>
              <Input
            ...
            <Card>
              <CardHeader className="pb-2">
                ...
                <CardDescription className="text-xs">
                  Adjust these audio features. Leave blank to ignore.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4 pt-4">
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    <Label htmlFor="target_danceability">
                      Target Danceability
                    </Label>
                    <span className="text-sm text-muted-foreground w-12 text-right">
                      {targetDanceability !== undefined
                        ? targetDanceability.toFixed(1)
                        : 'Any'}
                    </span>
                  </div>
                  <Slider
                  ...
                  {targetDanceability !== undefined && (
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => setTargetDanceability(undefined)}
                      className="text-xs"
                    >
                      Clear
                    </Button>
                ...
                <div className="space-y-2">
                  ...
                  <Slider
                    id="target_energy"
                    defaultValue={
                      targetEnergy !== undefined ? [targetEnergy] : [0.5]
                    }
                    value={
                      targetEnergy !== undefined ? [targetEnergy] : undefined
                    }
                  ...
                  {targetEnergy !== undefined && (
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => setTargetEnergy(undefined)}
                      className="text-xs"
                    >
                      Clear
                    </Button>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    <Label htmlFor="target_valence">
                      Target Valence (Positiveness)
                    </Label>
                    <span className="text-sm text-muted-foreground w-12 text-right">
                      {targetValence !== undefined
                        ? targetValence.toFixed(1)
                        : 'Any'}
                    </span>
                  </div>
                  <Slider
                    id="target_valence"
                    defaultValue={
                      targetValence !== undefined ? [targetValence] : [0.5]
                    }
                    value={
                      targetValence !== undefined ? [targetValence] : undefined
                    }
                  ...
                  {targetValence !== undefined && (
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => setTargetValence(undefined)}
                      className="text-xs"
                    >
                      Clear
                    </Button>
                ...
                <div className="space-y-2">
                  ...
                  {minPopularity !== undefined && (
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => setMinPopularity(undefined)}
                      className="text-xs"
                    >
                      Clear
                    </Button>
          ...
          <CardHeader className="flex flex-row justify-between items-center">
            <div>
              ...
              <CardDescription>
                Generated at{' '}
                {new Date(
                  currentDiscoveryQueue.generatedAt,
                ).toLocaleTimeString()}{' '}
                with {currentDiscoveryQueue.items.length} tracks.
              </CardDescription>
            </div>
                ...
                >
                  <div className="flex items-center space-x-4">
                    {item.album.images?.[0]?.url && (
                      <Image
                        ...
                        width={40}
                        height={40}
                        className="rounded object-cover ml-auto flex-shrink-0"
                      />
                    )}
                    <div>
                      ...
                      {item.recommendedBecause && (
                        <p className="text-xs text-sky-600 dark:text-sky-400">
                          Reason: {item.recommendedBecause}
                        </p>
                      )}
                      {item.sourceAlgorithm && (
                        <Badge variant="outline" className="mt-1 text-xs">
                          Source: {item.sourceAlgorithm}
                        </Badge>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center space-x-2">
                    ...
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => toast.info('Add to playback queue: TODO')}
                    >
                      Add to Queue
                    </Button>
                  </div>
      ...
      {currentDiscoveryQueue &&
        currentDiscoveryQueue.items.length === 0 &&
        !isLoadingRecommendations && (
          <Card className="mt-8">
            <CardHeader>
            ...
            <CardContent>
              <p>
                No recommendations found for the selected criteria. Try
                adjusting your seeds or parameters.
              </p>
            </CardContent>
          </Card>
        )}
      ...
      {savedPlaylists.length > 0 && (
        <Card className="mt-8">
          <CardHeader>
            ...
            <CardDescription>
              These playlists are saved in this session. Refreshing the page
              will clear them.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <ul className="space-y-2">
              {savedPlaylists.map((playlist) => (
                ...
                >
                  ...
                  <p className="text-sm text-muted-foreground">
                    {playlist.items.length} tracks, saved at{' '}
                    {new Date(playlist.savedAt).toLocaleTimeString()}
                  </p>
                </li>
              ))}
            </ul>

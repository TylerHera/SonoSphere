...
export default function DiscogsImportCard({
  release,
  accessToken: initialAccessToken,
}: DiscogsImportCardProps) {
  ...
  const mapDiscogsToVinylItem = (
    discogsRelease: DiscogsRelease,
  ): CreateVinylItemPayload => {
    ...
    } else if ((discogsRelease as any).artist) {
      // Fallback for older/different structures if any
    ...
    return {
      ...
      labels: discogsRelease.labels
        ? discogsRelease.labels.map((l) => ({ name: l.name, catno: l.catno }))
        : undefined, // Corrected from label
    };
  };
  ...
  const displayImageUrl =
    release.cover_image || release.thumb || placeholderImage(150, 150);
    ...
    <Card className="flex flex-col h-full relative">
      {isImported && (
        <div className="absolute inset-0 bg-green-900 bg-opacity-80 flex flex-col items-center justify-center z-10 rounded-md">
          ...
          <p className="text-white text-sm font-semibold text-center px-2">
            {error && error.startsWith('Item already')
              ? 'Already in Collection'
              : 'Imported!'}
          </p>
          {error && error.startsWith('Item already') && (
            <p className="text-xs text-gray-200">(Discogs ID match)</p>
          )}
        </div>
      )}
      {error && !isImported && (
      ...
      <CardContent className="flex-grow p-3 space-y-1">
        ...
        {release.labels && release.labels.length > 0 && (
          <p
            className="text-xs text-muted-foreground line-clamp-1"
            title={release.labels.map((l) => l.name).join(', ')}
          >
      ...
      <CardFooter className="p-2 border-t flex justify-between items-center">
        ...
        {release.uri && (
          <Button variant="ghost" size="icon" asChild title="View on Discogs">
            <Link
              href={`https://discogs.com${release.uri}`}
              target="_blank"

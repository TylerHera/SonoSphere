...
import { searchReleases, DiscogsSearchResult, DiscogsRelease } from '@/lib/api/discogs';
...
async function DiscogsSearchResults({ query, currentPage, accessToken }: { query: string; currentPage: number; accessToken: string | null }) {
  try {
    const searchData: DiscogsSearchResult = await searchReleases({ query: query, page: currentPage, per_page: 10 }); // smaller per_page for import UI
    
    if (!searchData.results || searchData.results.length === 0) {
        return <p className="text-center text-muted-foreground">No results found on Discogs for "{query}".</p>;
    }
      ...
      <>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
          {searchData.results.map(release => (
            <DiscogsImportCard key={release.id} release={release} accessToken={accessToken} />
          ))}
        </div>
        ...
        <p className="text-center text-sm text-muted-foreground mt-4">
          Page {searchData.pagination.page} of {searchData.pagination.pages} ({searchData.pagination.items} results on Discogs)
        </p>
      </>
    );
  } catch (error) {
    console.error("Failed to fetch Discogs search results for import:", error);
    return <p className="text-center text-destructive">Failed to load Discogs search results. Please ensure your Discogs API keys are set up and try again.</p>;
  }
}
...
export default async function ImportDiscogsPage({ searchParams }: ImportPageProps) {
    ...
    <div className="container mx-auto py-8 px-4">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          Search for releases on Discogs and add them to your local SonoSphere collection.
        </p>
      </div>
      <CollectionSearchForm searchPath="/collection/import" />
      {query ? (
        <Suspense fallback={<p className="text-center mt-6">Loading Discogs search results...</p>}>
          <DiscogsSearchResults query={query} currentPage={currentPage} accessToken={accessToken} />
        </Suspense>
      ) : (
        <p className="text-center text-muted-foreground mt-6">Search for releases on Discogs to see results.</p>
      )}
    </div>
  );
} 
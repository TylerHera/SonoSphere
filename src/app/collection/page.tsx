...
export default function CollectionPage() {
  ...
  const [statusFilter, setStatusFilter] = useState(searchParams.get('status') || ''); // ALL, OWNED, WISHLIST
  ...
  // Folder filter state
  const [folderFilter, setFolderFilter] = useState(searchParams.get('folder') || ''); // Added folderFilter state


  const apiBaseUrl = process.env.NEXT_PUBLIC_API_BASE_URL;

  const fetchCollection = useCallback(async (page: number, currentSearch?: string, currentStatus?: string, currentGenre?: string, currentSortBy?: string, currentSortOrder?: string, currentTags?: string, currentFolder?: string) => {
    ...
    if (currentStatus && currentStatus !== 'ALL') queryParams.append('status', currentStatus); // API expects OWNED or WISHLIST
    ...
    try {
      const response = await fetch(`${apiBaseUrl}/vinyl-items?${queryParams.toString()}`, {
        headers: {
  ...
  useEffect(() => {
    ...
    if (user) {
      fetchCollection(pageFromUrl, searchTerm, statusFilter, genreFilter, sortBy, sortOrder, tagsFilter, folderFilter);
    } else {
      setIsLoading(false);
      setError("Please log in to view your collection.");
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [user, session, apiBaseUrl]); // Only re-run if user/session/apiBaseUrl changes; fetchCollection handles params

  const handleFilterChange = () => {
    // Reset to page 1 when filters change
    setCurrentPage(1); 
    fetchCollection(1, searchTerm, statusFilter, genreFilter, sortBy, sortOrder, tagsFilter, folderFilter);
  };
  
  const handleTagFilterClick = (tag: string) => {
    setTagsFilter(tag); 
    setCurrentPage(1);
    fetchCollection(1, searchTerm, statusFilter, genreFilter, sortBy, sortOrder, tag, folderFilter);
  };

  const handleFolderFilterClick = (folder: string) => {
    setFolderFilter(folder);
    setCurrentPage(1);
    fetchCollection(1, searchTerm, statusFilter, genreFilter, sortBy, sortOrder, tagsFilter, folder);
  };
  ...
  const handleItemUpdated = () => {
    fetchCollection(currentPage, searchTerm, statusFilter, genreFilter, sortBy, sortOrder, tagsFilter, folderFilter); 
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`${apiBaseUrl}/vinyl-items/${itemToDelete}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${session.access_token}`,
        },
      ...
      fetchCollection(currentPage, searchTerm, statusFilter, genreFilter, sortBy, sortOrder, tagsFilter, folderFilter); // Refresh collection
    } catch (e: any) {
  ...
  const handleNextPage = () => {
    if (currentPage < totalPages) {
      ...
      fetchCollection(nextPage, searchTerm, statusFilter, genreFilter, sortBy, sortOrder, tagsFilter, folderFilter);
    }
  };
  ...
  const handlePreviousPage = () => {
    if (currentPage > 1) {
      ...
      fetchCollection(prevPage, searchTerm, statusFilter, genreFilter, sortBy, sortOrder, tagsFilter, folderFilter);
    }
  };

  if (isLoading && collectionItems.length === 0 && !error) {
    return <div className="p-4 md:p-6 text-center">Loading your collection...</div>;
  }

  if (error) {
    return <div className="p-4 md:p-6 text-center text-destructive">{error}</div>;
  }
    ...
    <div className="space-y-6 p-4 md:p-6">
      ...
      {itemToEdit && ( // Ensure itemToEdit is not null before rendering
        <EditVinylItemForm
          ...
          onCancel={() => { setShowEditForm(false); setItemToEdit(null); }}
        />
      )}

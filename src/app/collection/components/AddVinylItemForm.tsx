...
export function AddVinylItemForm({ accessToken, onFormSubmit, onCancel, isOpen }: AddVinylItemFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ title: '', artist_main: '', status: 'OWNED', custom_tags: [], folder: '' }); // Reset form including folder
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 max-h-[70vh] overflow-y-auto p-1 pr-3">
          ...
          <div>
            ...
            <Input id="artist_main" name="artist_main" value={formData.artist_main} onChange={handleChange} required />
          </div>
          <div>
          ...
          <div>
            ...
            <Input id="folder" name="folder" value={formData.folder || ''} onChange={handleChange} placeholder="e.g., Jazz Vocals, 70s Rock" />
          </div>
          <div>
            <Label htmlFor="year">Year</Label>
            <Input id="year" name="year" type="number" value={formData.year || ''} onChange={handleNumberChange} />
          </div>
           <div>
            <Label htmlFor="release_title">Release Title (if different from main title)</Label>
            <Input id="release_title" name="release_title" value={formData.release_title || ''} onChange={handleChange} />
          </div>
          <div>
            <Label htmlFor="genres">Genres (comma-separated)</Label>
            <Input id="genres" name="genres" value={formData.genres?.join(', ') || ''} onChange={handleArrayChange} />
          </div>
          <div>
            <Label htmlFor="styles">Styles (comma-separated)</Label>
            <Input id="styles" name="styles" value={formData.styles?.join(', ') || ''} onChange={handleArrayChange} />
          </div>
          <div>
          ...
          <div>
            ...
            <Input id="cover_url_large" name="cover_url_large" value={formData.cover_url_large || ''} onChange={handleChange} />
          </div>
          <div>

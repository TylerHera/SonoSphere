...
export default function ScrobblesPage() {
  ...
  const fetchLastfmProfileData = useCallback(async () => {
    if (user) {
      ...
      if (!data?.lastfm_username || !data?.lastfm_session_key) {
        setError(
          'Last.fm account not fully connected or username/session key not found. Please connect in Settings.',
        );
        setIsLoading(false);
        return false;
      }
  ...
  const fetchScrobbles = useCallback(async () => {
    ...
    setIsLoading(true);
    setError(null);
    const response = await getUserRecentTracks(
      lastfmUsername,
      ITEMS_PER_PAGE,
      currentPage,
    );
  ...
  if (isLoading && !recentTracks.length && !error) {
    return (
      <div className="p-4 md:p-6 text-center">Loading scrobble history...</div>
    );
  }
  ...
  if (!lastfmUsername && !isLoading) {
    return (
      <div className="p-4 md:p-6 text-center">
        <p>
          Please connect your Last.fm account in settings to view or add
          scrobbles.
        </p>
        <Button asChild className="mt-4">
          <Link href="/settings/connections">Go to Connections</Link>
    ...
    <div className="space-y-6 p-4 md:p-6">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold">
          Scrobble History {lastfmUsername && `(${lastfmUsername})`}
        </h1>
        {lastfmSessionKey && (
          <ManualScrobbleForm
      ...
      {recentTracks.length === 0 && !isLoading && (
        <div className="p-4 md:p-6 text-center">
          No scrobbles found for {lastfmUsername}. You can add one manually.
        </div>
      )}
            ...
            >
              <CardContent className="p-4 flex items-center space-x-4">
                <Image
                  src={
                    track.image.find(
                      (img: LastFM.Image) => img.size === 'large',
                    )?.['#text'] || placeholderImage(64, 64)
                  }

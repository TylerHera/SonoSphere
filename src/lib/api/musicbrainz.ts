...
const buildLuceneQuery = (
  ...
  return Object.entries(params)
    ...
    .map(
      ([key, value]) =>
        `${key}:${String(value).includes(' ') ? `"${value}"` : value}`,
    )
    .join(' AND ');
};

async function fetchMusicBrainzAPI<
  T extends
    | MusicBrainz.SearchResponseHeader
    | MusicBrainz.Release
...
>(
  ...
  const response = await fetch(url, {
    headers: {
      'User-Agent':
        MUSICBRAINZ_USER_AGENT || 'SonoSphere/0.1 ( mailto:YOUR_EMAIL_HERE )',
    },
  });
  ...
  if (!response.ok) {
    ...
    console.error(
      `MusicBrainz API Error (${response.status}) for ${url}: ${errorBody}`,
    );
    throw new Error(
      `MusicBrainz API request failed: ${response.statusText} - ${errorBody}`,
    );
  }
...
): Promise<MusicBrainz.CoverArtResponse | undefined> {
  try {
    ...
    if (response.status === 307 || response.status === 302) {
      // Temporary or Found redirect (often to image itself)
    ...
    console.error(
      `Failed to fetch/process cover art for ${entityType} ${mbid}: ${error.message}`,
    );
    return undefined;
  }
}

const getSmallestThumbnail = (
  coverArt?: MusicBrainz.CoverArtResponse,
): string | undefined => {
  if (!coverArt || !coverArt.images || coverArt.images.length === 0)
    return undefined;
  const frontImage = coverArt.images.find(
    (img) =>
      img.front && (img.types.includes('Front') || img.types.includes('front')),
  );
  const imageToUse = frontImage || coverArt.images[0];

  if (imageToUse.thumbnails) {
    if (imageToUse.thumbnails.small)
      return imageToUse.thumbnails.small.replace(/^http:/, 'https');
    if (imageToUse.thumbnails['250'])
      return imageToUse.thumbnails['250'].replace(/^http:/, 'https');
    if (imageToUse.thumbnails.large)
      return imageToUse.thumbnails.large.replace(/^http:/, 'https');
    if (imageToUse.thumbnails['500'])
      return imageToUse.thumbnails['500'].replace(/^http:/, 'https');
  }
  return imageToUse.image.replace(/^http:/, 'https');
};

export async function searchReleaseGroups(queryParams: {
  artist?: string;
  releasegroup?: string;
  query?: string;
  type?: MusicBrainz.ReleaseGroup['primary-type'];
  primarytype?: MusicBrainz.ReleaseGroup['primary-type'];
  limit?: number;
  offset?: number;
  ...
  if (queryParams.releasegroup)
    conditions.releasegroup = queryParams.releasegroup;
  ...
  const luceneQuery =
    buildLuceneQuery(conditions) +
    (query ? (buildLuceneQuery(conditions) ? ' AND ' : '') + query : '');

  return fetchMusicBrainzAPI<MusicBrainz.ReleaseGroupSearchResponse>(
    'release-group',
    {
      query: luceneQuery,
...
export async function searchUpcomingReleases(
  query?: string,
  daysAhead = 90,
): Promise<MusicBrainz.ReleaseGroup[]> {
...
export async function getArtistReleaseGroups(
  artistMbid: MusicBrainz.MBID,
  type?: MusicBrainz.ReleaseGroup['primary-type'],
): Promise<MusicBrainz.ReleaseGroup[]> {
  try {
    const data = await searchReleaseGroups({
      artist: artistMbid,
      type: type || 'album',
      limit: 50,
    });
    ...
    console.error(
      `Error fetching release groups for artist ${artistMbid}:`,
      error,
    );
    return [];
  }
...
export async function searchArtists(
  ...
  limit: number = 10,
  offset: number = 0,
): Promise<MusicBrainz.ArtistSearchResponse> {
  return fetchMusicBrainzAPI<MusicBrainz.ArtistSearchResponse>(
    'artist',
    { query, limit, offset },
    true,
  );
...
export async function searchRecordings(
  ...
  limit: number = 10,
  offset: number = 0,
): Promise<MusicBrainz.RecordingSearchResponse> {
  // Example query: "recording:Smells Like Teen Spirit AND artist:Nirvana"
  return fetchMusicBrainzAPI<MusicBrainz.RecordingSearchResponse>(
    'recording',
    { query, limit, offset },
    true,
  );
...
export async function getReleaseDetails(
  releaseMbid: MusicBrainz.MBID,
  inc: string = 'artist-credits+labels+recordings+release-groups',
): Promise<MusicBrainz.Release> {
  return fetchMusicBrainzAPI<MusicBrainz.Release>(`release/${releaseMbid}`, {
    inc,
  });
}
...
export async function getRecordingDetails(
  recordingMbid: MusicBrainz.MBID,
  inc: string = 'artist-credits+releases+release-groups',
): Promise<MusicBrainz.Recording> {
  return fetchMusicBrainzAPI<MusicBrainz.Recording>(
...
export async function getArtistDetails(
  artistMbid: MusicBrainz.MBID,
  inc: string = 'release-groups+aliases',
): Promise<MusicBrainz.Artist> {
  return fetchMusicBrainzAPI<MusicBrainz.Artist>(`artist/${artistMbid}`, {
    inc,
  });
}
...
export async function getReleaseGroupDetails(
  releaseGroupMbid: MusicBrainz.MBID,
  inc: string = 'artists+releases',
): Promise<MusicBrainz.ReleaseGroup> {
  return fetchMusicBrainzAPI<MusicBrainz.ReleaseGroup>(

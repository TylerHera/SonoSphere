...
export function parseLRC(lrcString: string): Musixmatch.LyricsLine[] {
  ...
  for (const line of lrcLines) {
    const timeStampMatches = Array.from(line.matchAll(/\[(\d{2,}):(\d{2})(?:[\.:](\d{2,3}))?\]/g));
    
    if (timeStampMatches.length > 0) {
      // Get text after the last timestamp
      const lastTimestampMatch = timeStampMatches[timeStampMatches.length - 1];
      const text = line.substring(lastTimestampMatch.index + lastTimestampMatch[0].length).trim();
      ...
      for (const match of timeStampMatches) {
        ...
        const totalTime = minutes * 60 + seconds + (match[3] && match[3].length === 3 ? hundredthsOrMillis / 1000 : hundredthsOrMillis / 100);

        if (!isNaN(totalTime) && text) { // Only add if text is not empty after timestamps
            lines.push({
                text,
                time: {
                    total: totalTime,
                    minutes,
                    seconds,
                    hundredths: match[3] && match[3].length === 2 ? hundredthsOrMillis : (match[3] && match[3].length === 3 ? Math.floor(hundredthsOrMillis / 10) : 0),
                },
            });
        }
      }
 ...
 */
export function findCurrentLyricLineIndex(lines: Musixmatch.LyricsLine[], currentTime: number): number {
